    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sub mt-1">{training.length} sesiones</p>
        </div>
      ...
      {training.length === 0 && <p className="text-muted py-12 text-center">Sin sesiones. Crea la primera.</p>}

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {training.map(s => {
          const exs = sessionEx.filter(e => e.session_id === s.id).sort((a, b) => a.ord - b.ord)
          return (
            <div key={s.id} className="bg-white rounded-2xl border border-line p-4">
              ...
              {s.goal && <div className="text-sm text-sub mb-2"><b>Objetivo:</b> {s.goal}</div>}
              {exs.length > 0 && (
                <div className="border-t border-line pt-2 mt-2 space-y-1">
                  {exs.map((e, i) => (
                    <div key={e.id} className="flex items-start gap-2 text-sm">
                      <span className="w-5 h-5 rounded bg-canvas text-sub flex items-center justify-center text-xs font-bold shrink-0">{i + 1}</span>
                      <div className="flex-1">
                        <span className="font-medium text-ink">{e.title}</span>
                        {e.video_url && <a href={e.video_url} target="_blank" className="text-red-500 ml-1">▶</a>}
                        <div className="text-xs text-muted">{[e.series && e.series + ' series', e.reps && e.reps + ' reps', e.weight].filter(Boolean).join(' · ')}</div>
                      </div>
                    </div>
                  ))}
                </div>
