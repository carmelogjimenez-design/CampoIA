    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sub mt-1">{matches.length} registrados</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {matches.map(m => (
          <div key={m.id} className="bg-white rounded-2xl border border-line p-4">
            <div className="flex justify-between items-center mb-2">
              <div>
                <div className="font-semibold text-ink">vs {m.rival}</div>
                <div className="text-xs text-sub">{m.date} · {getPlayerName(players, m.player_id)}</div>
              </div>
              <div className="text-right">
                <div className="font-display font-bold text-lg text-campo-blue">{m.result || '—'}</div>
                <div className="text-xs text-muted">{m.mins}'</div>
              </div>
            </div>
        ...
        {!matches.length && <p className="text-muted py-8">Sin partidos aún.</p>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4"
          ...
          <div className="bg-white rounded-2xl shadow-apple-lg w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="block text-xs font-bold text-sub mb-1">FECHA</label>
                <input type="date" className={inp} value={date} onChange={e => setDate(e.target.value)} /></div>
              <div><label className="block text-xs font-bold text-sub mb-1">RIVAL *</label>
                <input className={inp} value={rival} onChange={e => setRival(e.target.value)} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="block text-xs font-bold text-sub mb-1">RESULTADO</label>
                <input className={inp} placeholder="2-1" value={result} onChange={e => setResult(e.target.value)} /></div>
              <div><label className="block text-xs font-bold text-sub mb-1">MINUTOS</label>
                <input type="number" className={inp} value={mins} onChange={e => setMins(e.target.value)} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="block text-xs font-bold text-sub mb-1">GOLES</label>
                <input type="number" className={inp} value={goals} onChange={e => setGoals(e.target.value)} /></div>
              <div><label className="block text-xs font-bold text-sub mb-1">ASISTENCIAS</label>
                <input type="number" className={inp} value={assists} onChange={e => setAssists(e.target.value)} /></div>
            </div>
            {gk && (
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div><label className="block text-xs font-bold text-sub mb-1">GOLES ENCAJADOS</label>
                  <input type="number" className={inp} value={conceded} onChange={e => setConceded(e.target.value)} /></div>
                <div><label className="block text-xs font-bold text-sub mb-1">¿PORTERÍA A 0?</label>
