    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sub mt-1">{pending.length} pendientes</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h2 className="text-sm font-bold text-sub mb-3">PENDIENTES</h2>
          {pending.map(t => <TaskRow key={t.id} t={t} players={players} onToggle={toggle} />)}
          {!pending.length && <p className="text-muted text-sm">Sin pendientes.</p>}
        </div>
        <div>
          <h2 className="text-sm font-bold text-sub mb-3">COMPLETADAS</h2>
          {done.map(t => <TaskRow key={t.id} t={t} players={players} onToggle={toggle} />)}
          {!done.length && <p className="text-muted text-sm">Ninguna aún.</p>}
        </div>
      </div>
      ...
      {show && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4"
          ...
          <div className="bg-white rounded-2xl shadow-apple-lg w-full max-w-md p-6">
            ...
            <label className="block text-xs font-bold text-sub mb-1">ENLACE DE VÍDEO</label>
...
function TaskRow({ t, players, onToggle }: { t: Task; players: Player[]; onToggle: (t: Task) => void }) {
  return (
    <div className="bg-white rounded-xl border border-line p-3 mb-2 flex items-start gap-3">
      <button onClick={() => onToggle(t)}
              className={`w-6 h-6 rounded-md border-2 shrink-0 flex items-center justify-center mt-0.5 ${t.done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-line-strong'}`}>
        {t.done && '✓'}
      </button>
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-medium ${t.done ? 'line-through text-muted' : 'text-ink'}`}>{t.title || t.description}</div>
        <div className="flex gap-1.5 mt-1 flex-wrap">
          <span className="text-[10px] bg-canvas text-sub px-2 py-0.5 rounded-full">{getPlayerName(players, t.player_id)}</span>
          {t.type && <span className="text-[10px] bg-canvas text-sub px-2 py-0.5 rounded-full">{t.type}</span>}
