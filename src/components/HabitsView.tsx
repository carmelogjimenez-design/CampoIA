...
export default function HabitsView({ players, coachId }: Props) {
  ...
  return (
    <div>
      ...
      <p className="text-slate-500 mb-6">Check-ins de tus jugadores · sueño, ánimo, energía</p>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {checkins.map(c => (
          <div key={c.id} className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            {c.notes && <p className="text-sm text-slate-500 mt-2 italic">"{c.notes}"</p>}
          </div>
        ))}
        {!checkins.length && <p className="text-slate-400 py-8">Aún no hay check-ins de los jugadores.</p>}
      </div>
    </div>
  )
}
