    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">Comparte vídeos con comentarios para tus jugadores</p>
        </div>
        ...
        {items.map(v => (
          <div key={v.id} className="bg-white rounded-2xl border border-slate-200 p-4">
            <div className="flex justify-between items-start mb-1">
              ...
              <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{getPlayerName(players, v.player_id)}</span>
            </div>
            {v.comment && <p className="text-sm text-slate-600 mb-2">{v.comment}</p>}
            {v.video_url && <a href={v.video_url} target="_blank" className="text-sm text-campo-blue font-medium">▶ Ver vídeo</a>}
          </div>
        ))}
        {!items.length && <p className="text-slate-400 py-8">Sin análisis aún.</p>}
      </div>
      ...
      {show && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={e => e.target === e.currentTarget && setShow(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <label className="block text-xs font-bold text-slate-600 mb-1">ENLACE DE VÍDEO (YouTube, Drive...)</label>
