...
import { Player, MealPlan, MealPlanItem } from '../types/database'
import { DAYS, MEAL_TYPES, getActivePlan, getTodayProgress, updateMealItem, deleteMealItem, addMealItem, deletePlan } from '../lib/mealPlan'
import { initials } from '../lib/players'
import Modal from './Modal'

interface Props { players: Player[]; coachId: string; onImport: (playerId: string) => void }

interface PlanState { playerId: string; plan: MealPlan; items: MealPlanItem[]; done: number; total: number }

export default function MealPlanManager({ players, coachId, onImport }: Props) {
  ...
  async function load() {
    ...
    for (const p of players) {
      const active = await getActivePlan(p.id)
      if (active) {
        const prog = await getTodayProgress(p.id, active.items)
        out.push({ playerId: p.id, plan: active.plan, items: active.items, done: prog.done, total: prog.total })
      }
    }
    setPlans(out)
  }
  useEffect(() => { load() }, [players])

  if (!plans.length) return null

  return (
    <div className="mb-6">
      <div className="eyebrow mb-3">Planes de alimentación activos</div>
      ...
      {editing && <PlanEditor ps={editing} coachId={coachId}
        onClose={() => setEditing(null)}
        onChanged={load}
        onReplace={() => { setEditing(null); onImport(editing.playerId) }} />}
    </div>
  )
}

function PlanEditor({ ps, coachId, onClose, onChanged, onReplace }: { ps: PlanState; coachId: string; onClose: () => void; onChanged: () => void; onReplace: () => void }) {
